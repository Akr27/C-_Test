    ...
    public class Calculator
    {
        ...
        private Boolean validate(string expression)
        {
            ...
            for (int i = 0; i < expression.Length - 1; i++)
            {
                ...
                {
                    if (tempArray[i + 1] == '+' || tempArray[i + 1] == '*' || tempArray[i + 1] == '/' || tempArray[i + 1] == '^' || tempArray[i + 1] == '%' || tempArray[i + 1] == '.')
                    {
                        return false;
                    }
                }
        ...
        public List<string> joinNumbers(List<string> expList)
        {
            if (check(expList))
            {
                for (int i = 0; i < expList.Count - 1; i++)
                {
                    ...
                    {
                        double tempNum = 10 * double1 + double2;
                        expList[i] = tempNum.ToString();
        ...
        {
            if (expList[0] == "-" && double.TryParse(expList[1],out double res))
            {
            ...
            if (checkNegative(expList))
            {
                for (int i = 1; i < expList.Count - 1; i++)
                {
                    if (expList[i] == "-" && !double.TryParse(expList[i - 1], out double temp2) && double.TryParse(expList[i+1], out double res2))
                    {
        ...
        public List<string> solveBrackets(List<string> expList)
        {
            ...
            else
            {
                ...
                if (firstInd == 0)
                {
                    ...
                    {
                        tempList.Add(subAns.ToString());
                        if(double.TryParse(expList[lastInd+1],out double x)) { tempList.Add("*"); }
                ...
                else
                {
                    if (lastInd == expList.Count - 1)
                    {
                        ...
                        if (double.TryParse(expList[firstInd-1], out double y)) { tempList.Add("*"); }
                        tempList.Add(subAns.ToString());
                    }
                    else
                    {
                        for (int i = 0; i < firstInd; i++) { tempList.Add(expList[i]); }
                        if (double.TryParse(expList[firstInd - 1], out double a)) { tempList.Add("*"); }
                        tempList.Add(subAns.ToString());
                        if (double.TryParse(expList[lastInd + 1], out double b)) { tempList.Add("*"); }
                        for (int i = lastInd + 1; i < expList.Count; i++) { tempList.Add(expList[i]); }
                    }
                }
            }
